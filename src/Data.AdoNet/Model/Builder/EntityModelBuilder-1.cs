  ...
  where TEntity : class
{
  ...
  private MemberInfo? _concurrencyToken;
  ...
  public override void AddPrimaryKeyColumns()
  {
    ...
    foreach (var memberComponent in primaryKeyMemberComponents)
    {
      ...
      if (memberComponent.Member == _concurrencyToken)
      {
        _entity.ConcurrencyToken = column;
        ((ColumnModel)column).IsConcurrencyToken = true;
      }
  ...
  public IEntityModelBuilder<TEntity> WithKey(params string[] keyNames)
  {
    ...
    if (keyNames.Any(string.IsNullOrWhiteSpace))
      throw new ArgumentException($"'{nameof(keyNames)}' should not contain null or whitespace strings.");
  ...
  public IEntityModelBuilder<TEntity> WithConcurrencyToken<TMember>(Expression<Func<TEntity, TMember>> expression)
  {
    if (expression is null)
      throw new ArgumentNullException(nameof(expression));
    ...
    _concurrencyToken = member;
    return this;
  }

  public IEntityModelBuilder<TEntity> WithConcurrencyToken<TMember>(string memberName)
  {
    if (string.IsNullOrWhiteSpace(memberName))
      throw new ArgumentException($"'{nameof(memberName)}' cannot be null or whitespace.", nameof(memberName));

    _concurrencyToken = GetMember(EntityType, memberName);
    return this;
  }
