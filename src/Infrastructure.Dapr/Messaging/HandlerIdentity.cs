...
namespace CodeArchitects.Platform.Infrastructure.Dapr.Messaging
{
  ...
  /// </summary>
  public readonly struct HandlerIdentity : IEquatable<HandlerIdentity>
  {
    /// <summary>
    /// Creates an instance of <see cref="HandlerIdentity"/>.
    /// </summary>
    ...
    public HandlerIdentity(string? busName, string? topic, Type messageType)
    {
      BusName = busName;
      Topic = topic;
    ...
    public readonly bool Equals(HandlerIdentity other)
      => BusName == other.BusName
      && Topic == other.Topic
      && MessageType == other.MessageType;

    public readonly override bool Equals(object? obj)
      => obj is HandlerIdentity other && Equals(other);

    public readonly override int GetHashCode()
      => HashCode.Combine(BusName, Topic, MessageType);

    /// <summary>
    /// Indicates whether the <see cref="HandlerIdentity"/> instance on the left-hand side is equal to the <see cref="HandlerIdentity"/> instance on the right-hand side.
    /// </summary>
    /// <param name="left">The left-hand side <see cref="HandlerIdentity"/>.</param>
    /// <param name="right">The right-hand side <see cref="HandlerIdentity"/>.</param>
    ...
    /// <summary>
    /// Indicates whether the <see cref="HandlerIdentity"/> instance on the left-hand side is not equal to the <see cref="HandlerIdentity"/> instance on the right-hand side.
    /// </summary>
    /// <param name="left">The left-hand side <see cref="HandlerIdentity"/>.</param>
    /// <param name="right">The right-hand side <see cref="HandlerIdentity"/>.</param>
