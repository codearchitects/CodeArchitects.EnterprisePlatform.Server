...
IActivityManager<TestActor> activityManager = ActivityManager<TestActor>.Create(descriptor);

builder.Services.AddControllers();
builder.Services.AddDaprClient();
builder.Services.AddActors(options =>
{
  options.Actors.Add(new ActorRegistration(ActorTypeInformation.Get(hostEmitResult.ClassType, nameof(TestActor))));
});
builder.Services.AddSingleton(typeof(ITestActorFactory), actorFactoryType);
