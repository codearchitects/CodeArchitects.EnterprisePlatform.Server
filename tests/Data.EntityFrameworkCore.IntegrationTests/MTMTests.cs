using CodeArchitects.Platform.Data.EntityFrameworkCore.Model.MTM;
...
{
  public class MTMTests : EFCoreIntegrationTest<MTMDbContext>
  {
    private readonly Repository<Primary, Guid> _sut;

    public MTMTests(ITestOutputHelper output)
      : base(output)
    {
      _sut = new Repository<Primary, Guid>(Context, Context.Primaries);
    }

    protected override MTMDbContext CreateContext(DbContextOptions<MTMDbContext> options) => new MTMDbContext(options);
